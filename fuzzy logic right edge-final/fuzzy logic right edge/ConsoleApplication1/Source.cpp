...

{
	...
	S1.set_name("Sensor7");
	S1.set_memval({ 0,500,200,800,500,1000 });
	S2.set_name("Sensor6");
	S2.set_memval({ 0,500,200,800,500,1000 });
	//cout << S1.Near.values[1] << endl;
	//cout << S2.Far.values[0] << endl;

	/* create output membership function for left wheel and right wheel*/
	w1.set_name("Left Wheel");
	w1.set_memval({ 0,100,100,200,200,250 });
	w2.set_name("Right Wheel");
	w2.set_memval({ 0,100,100,200,200,250 });
	...
	while (true)
	{
		...
		vector <double> mem_valS2 = mf.calc_mem_val(1, sonarRange[6], { 0,0,0 }, { 0,0,0 }, { 0,0,0 }, S2.Near.values, S2.Good.values, S2.Far.values); //-sin(0.87) *
